      ...
      <div className="bg-white w-full max-w-2xl rounded-2xl shadow-2xl border border-slate-200 overflow-hidden my-8 animate-in fade-in zoom-in-95 duration-200">
        ...
        <div className="px-6 py-4 border-b border-slate-100 flex items-center justify-between bg-slate-50">
          <div className="flex items-center gap-2">
            ...
            <div>
              <h2 className="font-bold text-sm text-slate-900">AI Safety Assessment Result</h2>
              <p className="text-[11px] text-slate-500">Evaluated at {new Date(result.evaluatedAt).toLocaleTimeString()}</p>
            </div>
          </div>
          <button
            onClick={onClose}
        ...
        <div className="p-6 space-y-6 max-h-[75vh] overflow-y-auto">
          ...
          <div className="p-6 rounded-2xl border border-slate-200/80 bg-slate-50/50 flex flex-col sm:flex-row items-center justify-between gap-6">
            <div className="space-y-2 text-center sm:text-left">
              ...
              <h3 className="text-xl font-extrabold text-slate-900">{result.summary}</h3>
              <p className="text-xs text-slate-600 max-w-md leading-relaxed">{result.explanation}</p>
            </div>

            {/* Circular / Large Score Badge */}
            <div className="flex flex-col items-center justify-center shrink-0">
              <div className="relative w-28 h-28 rounded-full bg-white shadow-md border-4 border-slate-100 flex flex-col items-center justify-center">
              ...
              <span className="text-[11px] text-slate-500 mt-2 font-medium">CPTED Vulnerability Index</span>
            </div>
          </div>
          ...
          <div className="space-y-3">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
              ...
              <div className="p-3.5 bg-rose-50/50 border border-rose-100 rounded-xl space-y-2">
                ...
                {amplifiers.length === 0 ? (
                  <p className="text-xs text-slate-500">No major negative risk amplifiers detected.</p>
                ) : (
                  <div className="space-y-1.5">
                    {amplifiers.map((f, i) => (
                      <div key={i} className="p-2 bg-white rounded-lg border border-rose-200/60 text-xs">
                        <div className="flex items-center justify-between font-bold text-slate-800">
                          <span>{f.factor}</span>
                          <span className="text-rose-600">+{f.scoreImpact} pts</span>
                        </div>
              ...
              <div className="p-3.5 bg-emerald-50/50 border border-emerald-100 rounded-xl space-y-2">
                ...
                {buffers.length === 0 ? (
                  <p className="text-xs text-slate-500">No major positive environmental buffers present.</p>
                ) : (
                  <div className="space-y-1.5">
                    {buffers.map((f, i) => (
                      <div key={i} className="p-2 bg-white rounded-lg border border-emerald-200/60 text-xs">
                        <div className="flex items-center justify-between font-bold text-slate-800">
                          <span>{f.factor}</span>
                          <span className="text-emerald-700">{f.scoreImpact} pts</span>
                        </div>
                        <p className="text-[11px] text-slate-500 mt-0.5">{f.description}</p>
                      </div>
                    ))}
        ...
        <div className="p-4 sm:p-6 border-t border-slate-100 bg-slate-50 flex flex-wrap items-center justify-between gap-3">
          <div className="flex items-center gap-2">
            <button
              onClick={handleShare}
              className="inline-flex items-center gap-1.5 px-3 py-2 bg-white border border-slate-200 text-slate-700 rounded-lg text-xs font-semibold hover:bg-slate-100 transition-colors"
            >
